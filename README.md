...
- 휴가는 열외 입력에서 `휴가` 카테고리로 등록하며 보고 날짜의 열외 인원으로 반영합니다.
...
- 시작일과 종료일을 같은 날짜로 두면 하루 열외로 저장됩니다.
- 시작일과 종료일이 다르면 기간 내 날짜별 열외 기록을 한 번에 저장합니다.
- 휴가도 열외 인원에 포함되므로 별도 탭이 아니라 열외 입력에서 카테고리를 `휴가`로 선택해 등록합니다.

명단 확인 방식:

- `당일` 버튼은 보고 날짜의 열외만 보여줍니다.
- `달 단위` 버튼은 보고 날짜가 속한 월의 열외를 보여줍니다.
- `전체`, `휴가`, `외박` 등 카테고리 버튼으로 필요한 열외구분만 필터링할 수 있습니다.

중복 방지 방식:

- DB에서 `unique(date, member_id)` 제약을 사용합니다.
- 같은 날짜에 같은 인원을 다시 저장하면 해당 날짜의 열외 기록을 갱신합니다.
...
| 열외 | 선택 날짜의 `daily_exceptions` 등록 인원 수. `휴가` 카테고리도 열외에 포함 |
| 현재원 | 총원 - 열외 |
| 현재원 명단 | active 인원 전체 - 선택 날짜 열외자 |
| 열외내용 | 열외구분별 그룹화. 휴가는 `휴가` 그룹에 포함 |
...
- 기간으로 저장한 열외는 날짜별 열외 기록으로 생성되며, 보고 날짜에 해당하는 기록만 보고문에 포함합니다.
- 휴가는 열외 입력에서 `휴가` 카테고리로 등록하며 현재원에서 제외됩니다.
